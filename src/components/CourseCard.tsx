...
export function CourseCard({ course, fill }: { course: Course; fill?: boolean }) {
  const off = discountPercent(course.price, course.originalPrice);
  return (
    <article
      className={`card-surface flex flex-col ${
        fill ? "w-full min-w-0" : "min-w-[240px] w-[min(280px,82vw)] md:w-[300px] shrink-0 snap-start"
      }`}
    >
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        ...
        <div className="mt-auto tracking-tight">
          <p className="mb-0 text-xl sm:text-2xl font-bold text-primary">{formatInr(course.price)}</p>
          <p className="text-muted-foreground line-through mb-0 text-base sm:text-lg">{formatInr(course.originalPrice)}</p>
        </div>
